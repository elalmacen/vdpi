...
if (have_comments()) {
	...
	if (comments_open()) {
		echo '<div class="comments-wrap">' . "\n";
			echo '<h4 class="comment-section-title">' . sprintf(_n('<span class="comment-count">S&eacute; el primero en comentar</span> <span class="comment-count-more">en "%2$s"</span>', '<span class="comment-count">S&eacute; el primero en comentar</span> <span class="comment-count-more">en "%2$s"</span>', $comment_count, 'mh-newsdesk'), number_format_i18n($comment_count), get_the_title()) . '</h4>' . "\n";
		echo '</div>' . "\n";
	}
}
