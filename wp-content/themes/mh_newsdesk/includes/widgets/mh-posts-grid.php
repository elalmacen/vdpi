...
/***** MH Posts Grid (Homepage) Widget *****/

class mh_newsdesk_posts_grid extends WP_Widget {
    function mh_newsdesk_posts_grid() {
        $widget_ops = array('classname' => 'mh_newsdesk_posts_grid', 'description' => __('Display posts as grid on front page for use in "Home 1" or "Home 4" widget areas.', 'mh-newsdesk'));
        $this->WP_Widget('mh_newsdesk_posts_grid', __('MH Posts Grid (Homepage)', 'mh-newsdesk'), $widget_ops);
    }
    function widget($args, $instance) {
		...
		<div class="mh-fp-grid-widget clearfix"><?php
			while ($widget_loop->have_posts()) : $widget_loop->the_post();
				...
				if (($counter >= 1) && ($counter <= $postcount)) {
