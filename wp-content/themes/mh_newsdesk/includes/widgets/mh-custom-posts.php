...
class mh_newsdesk_custom_posts extends WP_Widget {
    function mh_newsdesk_custom_posts() {
        $widget_ops = array('classname' => 'mh_newsdesk_custom_posts', 'description' => __('Display posts from any category or tag.', 'mh-newsdesk'));
        $this->WP_Widget('mh_newsdesk_custom_posts', __('MH Custom Posts', 'mh-newsdesk'), $widget_ops);
    }
				...
				if ($counter == 1 && $excerpt == 'first' || $excerpt == 'all') : ?>
					<article class="cp-wrap cp-large clearfix">
						...
						<div class="cp-thumb-xl">
							<a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>"><?php if (has_post_thumbnail()) { the_post_thumbnail('cp-thumb-xl'); } else { echo '<img src="' . get_template_directory_uri() . '/images/placeholder-thumb-xl.jpg' . '" alt="No Picture" />'; } ?></a>
						</div>
                        <?php endif; ?>
						<?php if ($date == 0 && $post_date) { ?>
							<p class="entry-meta"><?php echo sprintf(_x('%s', 'post date', 'mh-newsdesk'), '<span class="updated">' . get_the_date() . '</span>'); ?></p>
						<?php } ?>
						<h3 class="cp-title-xl"><a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>" rel="bookmark"><?php the_title(); ?></a></h3>
					...
					<article class="cp-wrap cp-small clearfix">
						<div class="cp-thumb-small">
							<a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>"><?php if (has_post_thumbnail()) { the_post_thumbnail('cp-thumb-small'); } ?></a>
						</div>
						<?php if ($date == 0 && $post_date) { ?>
							<p class="entry-meta"><?php echo sprintf(_x('%s', 'post date', 'mh-newsdesk'), '<span class="updated">' . get_the_date() . '</span>'); ?></p>
						<?php } ?>
